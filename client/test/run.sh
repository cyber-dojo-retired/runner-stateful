...
readonly TEST_LOG=${RUNNER_STATEFUL_COVERAGE_ROOT}/test.log

mkdir -p ${RUNNER_STATEFUL_COVERAGE_ROOT}
cd ${MY_DIR}/src
...
cd ${MY_DIR} \
  && ruby ./check_test_results.rb \
       ${TEST_LOG} \
       ${RUNNER_STATEFUL_COVERAGE_ROOT}/index.html \
          > ${RUNNER_STATEFUL_COVERAGE_ROOT}/done.txt
