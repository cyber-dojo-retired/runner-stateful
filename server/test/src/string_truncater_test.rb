...
class StringTruncaterTest < TestBase
  ...
  test '4D1',
  %w( empty string is not truncated ) do
    s = ''
  ...
  test '5A7',
  %w( string of less than 10k is not truncated ) do
    s = '@' * (max_length - 1)
    assert_equal s, truncated(s)
  end
  ...
  test 'D8F',
  %w( string of exactly 10k is not truncated ) do
    s = '@' * (max_length)
    assert_equal s, truncated(s)
  end
  ...
  test 'A48',
  %w( string of greater than 10k is truncated
